...
#include <pcl/compact/impl/compact_monitor_queue.hpp>
#include <pcl/compact/impl/compact_lru_cache.hpp>

#include <pcl/compact/visualization/camera.h>
#include <pcl/compact/visualization/common.h>
#include <pcl/compact/visualization/object.h>
#include <pcl/compact/visualization/scene.h>
#include <pcl/compact/visualization/compact_cloud.h>
...
boost::condition CompactCloud::pcd_queue_ready;
boost::mutex CompactCloud::pcd_queue_mutex;

boost::shared_ptr<boost::thread> CompactCloud::pcd_reader_thread;
//MonitorQueue<std::string> CompactCloud::pcd_queue;

//std::map<std::string, vtkSmartPointer<vtkPolyData> > CompactCloud::cloud_data_cache;
CompactCloud::CloudDataCache CompactCloud::cloud_data_cache(524288);
boost::mutex CompactCloud::cloud_data_cache_mutex;

CompactCloud::PcdQueue CompactCloud::pcd_queue;

void
CompactCloud::pcdReaderThread ()
{
...
// Operators
// -----------------------------------------------------------------------------
CompactCloud::CompactCloud (std::string name, boost::filesystem::path& tree_root) :
    Object (name), display_depth_ (1), points_loaded_ (0), data_loaded_(0), render_camera_(NULL), lod_pixel_threshold_(10000)
{

  // Create the pcd reader thread once for all compact nodes
  if (CompactCloud::pcd_reader_thread.get() == NULL)
  {
//    CompactCloud::pcd_reader_thread = boost::shared_ptr<boost::thread>(new boost::thread(&CompactCloud::pcdReaderThread, this));
    CompactCloud::pcd_reader_thread = boost::shared_ptr<boost::thread>(new boost::thread(&CompactCloud::pcdReaderThread));
  }
...
void
CompactCloud::render (vtkRenderer* renderer)
{
